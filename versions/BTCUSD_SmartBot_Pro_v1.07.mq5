...
#property description "Grid Risk Adjustment - Divise risque par nb niveaux - VPS Windows Server 2022"
...
int OnInit()
{
   ...
   Print("═══════════════════════════════════════════════════════");
   Print("🤖 BTCUSD SmartBot Pro v1.07 - Initialisé avec succès");
   Print("═══════════════════════════════════════════════════════");
...
double CalculateGridLot(double baseLot, int gridLevel)
{
   ...
   // Normaliser selon le lot step du broker
   if(lotStep > 0)
...
bool OpenGridLevel(string direction, double price, double atr, int gridLevel)
{
   ...
   if(gridLevel == 0)
   {
      // CORRECTION v1.07 : Ajuster le risque si Grid activé
      double adjustedRisk = InpRiskPercent;

      if(InpUseGrid && InpMaxGridLevels > 1)
      {
         // Diviser le risque par le nombre de niveaux pour éviter sur-exposition
         adjustedRisk = InpRiskPercent / InpMaxGridLevels;

         Print("⚠️ Grid activé : Risque ajusté de ", DoubleToString(InpRiskPercent, 2),
               "% à ", DoubleToString(adjustedRisk, 2), "% (÷", InpMaxGridLevels, " niveaux)");
      }

      // Premier niveau : calculer le lot basé sur le risque ajusté
      double balance = accountInfo.Balance();
      double riskMoney = balance * adjustedRisk / 100.0;

      double stopLoss = direction == "BUY" ? price - slDistance : price + slDistance;

      double slDistanceAbs = MathAbs(price - stopLoss);
      if(slDistanceAbs == 0) return false;

      double tickValue = symbolInfo.TickValue();
      double tickSize = symbolInfo.TickSize();

      baseLot = riskMoney / (slDistanceAbs / tickSize * tickValue);
